...
static const unsigned int fsm_rates[] = {8000,  16000, 32000,
                                         44100, 48000, 96000};
...
static const struct snd_kcontrol_new fsm_snd_controls[] = {
    ...
    SOC_SINGLE_EXT("FSM_Stop", SND_SOC_NOPM, 0, 1, 0, fsm_stop_get,
                   fsm_stop_put),
    SOC_SINGLE_EXT("FSM_AMP_ON", SND_SOC_NOPM, 0, 1, 0, fsm_amp_get,
                   fsm_amp_put),
    SOC_SINGLE_EXT("FSM_Rotation", SND_SOC_NOPM, 0, 360, 0, fsm_rotation_get,
                   fsm_rotation_put),
    SOC_SINGLE_EXT("FSM_Ultrasound_Enable", SND_SOC_NOPM, 0, 1, 0,
                   fsm_ultrasound_get, fsm_ultrasound_put),
    SOC_SINGLE_EXT("FSM_SW_GPIO", SND_SOC_NOPM, 0, 0x3, 0, fsm_spksw_gpio_get,
                   fsm_spksw_gpio_put),
};
...
snd_soc_fsm_get_dapm(struct snd_soc_codec *codec) {
...
static int fsm_set_sysclk(struct snd_soc_dai *codec_dai, int clk_id,
...
static int fsm_hw_params(struct snd_pcm_substream *substream,
  ...
  pr_info("srate:%d, chn:%d, chn_size:%d, bclk:%d", srate, channel, chn_size,
          bclk);
...
static int fsm_trigger(struct snd_pcm_substream *substream, int cmd,
                       struct snd_soc_dai *dai) {
  switch (cmd) {
...
int fsm_codec_register(struct device *dev, int id) {
  ...
  ret = snd_soc_register_codec(dev, &soc_codec_dev_fsm, &fsm_aif_dai[id], 1);
  if (ret < 0) {
