...
fsm_config_t *fs19xx_get_config(void) { return &g_fsm_config; }
#ifdef __KERNEL__
...
int fsm_burst_write(fsm_dev_t *fsm_dev, uint8_t reg, uint8_t *data, int len) {
  ...
  if (len >= 4) {
    if (g_fsm_config.i2c_debug) {
      pr_addr(info, "%02X<-%02X %02X %02X %02X", reg, data[0], data[1], data[2],
              data[3]);
    }
  }
...
static int fsm_check_dev_type(fsm_dev_t *fsm_dev) {
  ...
  if (fsm_dev->is1820) {
    if (LOW8(fsm_dev->version) != HIGH8(dev_type)) {
      pr_addr(err, "type:%02X not match version:%02X", HIGH8(dev_type),
              HIGH8(fsm_dev->version));
      return -EINVAL;
    }
    return 0;
  }
...
int fsm_init_dev_list(fsm_dev_t *fsm_dev) {
  ...
  if (ret) {
    pr_addr(err, "type(%04X) not matched version(%04X)", dev_list->dev_type,
            fsm_dev->version);
    fsm_dev->dev_list = NULL;
    return ret;
  }
  ...
  if ((dev_list->preset_ver & BIT(15)) == 0) { // BIT_15 must be 1
    pr_addr(err, "invalid preset version:%04X", dev_list->preset_ver);
    return -EINVAL;
  }
  pr_addr(debug, "bus  : %d, len: %d", dev_list->bus, dev_list->len);
  pr_addr(debug, "type : %04X, npreset: %d", dev_list->dev_type,
          dev_list->npreset);
  pr_addr(debug, "scene: eq:%04X, reg:%04X", dev_list->eq_scenes,
          dev_list->reg_scenes);
...
int fsm_init_info(fsm_dev_t *fsm_dev) {
  ...
  pr_addr(info, "tmax:%d, tcoef:%d, tsel:%d", fsm_dev->tmax, fsm_dev->tcoef,
          fsm_dev->tsel);
  ...
  pr_addr(info, "pos:%02X, rs_ratio:%d", fsm_dev->pos_mask,
          fsm_dev->compat.RS2RL_RATIO);
  pr_addr(info, "spkr:%d, rapp:%d", fsm_dev->spkr, fsm_dev->rapp);

  return 0;
}

void *fsm_get_presets(void) { return g_fsm_config.preset; }
...
int fsm_parse_preset(const void *data, uint32_t size) {
  ...
  pr_info("date    : %4u%02u%02u-%02u%02u", hdr->date.year, hdr->date.month,
...
int fsm_swap_channel(fsm_dev_t *fsm_dev, int next_angle) {
  ...
  if ((g_fsm_config.dev_count == 1) || (fsm_dev->pos_mask == FSM_POS_MONO)) {
    chs12 = 3;
  }
  if (fsm_dev->is19xx) {
    i2sctrl = 0x17;
...
int fsm_ops_dummy(fsm_dev_t *fsm_dev) { return 0; }
...
int fsm_stub_switch_preset(fsm_dev_t *fsm_dev) {
  ...
  pr_addr(debug, "%s switching", (cfg->force_scene ? "force" : "auto"));
...
int fsm_dev_count(void) { return g_fsm_config.dev_count; }
