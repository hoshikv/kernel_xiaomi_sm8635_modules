...
mi_disp_timming_switch_delayed_work_handler(struct kthread_work *work) {
  ...
  if (dsi_panel->panel_initialized) {
    ...
    DISP_ERROR("Panel not initialized, don't send "
...
int mi_dsi_update_switch_cmd_N3(struct dsi_panel *panel, u32 cmd_update_index,
                                u32 index) {
  ...
  for (i = 0; i < cmd_update_count; i++) {
    ...
    if (info && info->mipi_address == 0x6C) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, index, info, &dc_6C_cfg,
                                  sizeof(dc_6C_cfg));
    }
    info++;
  }
...
int mi_dsi_update_switch_cmd_N8(struct dsi_panel *panel, u32 cmd_update_index,
                                u32 index) {
  struct dsi_cmd_update_info *info = NULL;
  struct dsi_display_mode *cur_mode = NULL;
  int i = 0;
  ...
  for (i = 0; i < cmd_update_count; i++) {
    ...
    if (info && info->mipi_address == 0x35) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, index, info, doze_cmd, size);
    }
    info++;
  }
...
int mi_dsi_set_switch_cmd_before(struct dsi_panel *panel, int fps_mode) {
  ...
  if ((last_fps == 10 && (fps_mode == DDIC_MODE_AUTO)) ||
      ((fps_mode == DDIC_MODE_IDLE) &&
       (mi_timing->ddic_min_refresh_rate == 10) &&
  ...
  if (last_fps == 120 && ((last_fps_mode == DDIC_MODE_AUTO) ||
    ...
    mi_dsi_update_switch_cmd_N3(panel,
  ...
  if (rc)
    DSI_INFO("[%s] N3 failed to send DSI_CMD_SET_MI_TIMING_SWITCH BEFOR cmds, "
             "rc=%d\n",
             panel->name, rc);
  ...
  for (i = 0; i < cmd_update_count; i++) {
    ...
    if (info && info->mipi_address == 0x26) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_TIMING_SWITCH,
                                  info, &gamma_26_cfg, sizeof(gamma_26_cfg));
    } else if (info && info->mipi_address == 0xC0) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_TIMING_SWITCH,
                                  info, &gamma_c0_cfg, sizeof(gamma_c0_cfg));
    } else if (info && info->mipi_address == 0xDF) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_TIMING_SWITCH,
                                  info, &gamma_df_cfg, sizeof(gamma_df_cfg));
    }
    info++;
  }
...
static inline bool mi_dsi_update_BA_reg(struct dsi_panel *panel, u8 *val,
                                        u8 size) {
  ...
  else if ((mi_timing->ddic_mode == DDIC_MODE_AUTO) ||
           (fps_mode == DDIC_MODE_QSYNC))
    cur_framerate = mi_timing->sf_refresh_rate;

  DISP_DEBUG("ddic mode:%d, last fps:%d, cur_fps:%d\n", mi_timing->ddic_mode,
             last_framerate, cur_framerate);
...
int mi_dsi_update_flat_mode_on_cmd(struct dsi_panel *panel,
  ...
  for (i = 0; i < cmd_update_count; i++) {
    if (info && info->mipi_address == 0xD5) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, &update_D5_cfg,
...
int mi_dsi_update_timing_switch_and_flat_mode_cmd(struct dsi_panel *panel,
  ...
  if (mi_get_panel_id_by_dsi_panel(panel) != N2_PANEL_PA &&
      mi_get_panel_id_by_dsi_panel(panel) != N1_PANEL_PA) {
    DISP_DEBUG(
        "this project not need to dynamically update these parameters\n");
    return 0;
  }
  ...
  case DSI_CMD_SET_TIMING_SWITCH:
    ...
    is_update_BA_reg = mi_dsi_update_BA_reg(panel, update_BA_reg_val,
                                            sizeof(update_BA_reg_val));
    break;
  ...
  DISP_DEBUG(
      "panel id is 0x%x, gamma_2Freg_cfg is 0x%x, gamma_26reg_cfg is 0x%x",
  ...
  for (i = 0; i < cmd_update_count; i++) {
    ...
    if (info && info->mipi_address == 0x2F) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, &gamma_2F_cfg,
                                  sizeof(gamma_2F_cfg));
    }

    if (info && info->mipi_address == 0x26) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, &gamma_26_cfg,
                                  sizeof(gamma_26_cfg));
    }
  ...
  if (mi_cfg->nedd_auto_update_gamma) {
    ...
    if (last_refresh_rate &&
        (ktime_to_us(cur_time - mi_cfg->last_mode_switch_time) <=
         vsync_period_us)) {
      DISP_INFO(
          "sleep before update gamma, vsync_period = %d, diff_time = %llu",
          vsync_period_us,
          ktime_to_us(cur_time - mi_cfg->last_mode_switch_time));
  ...
  if (mi_cfg->in_fod_calibration) {
    ...
    if (bl_lvl != 0) {
      if ((mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N3_PANEL_PA ||
           mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N9_PANEL_PA) &&
          is_hbm_fod_on(panel)) {
        DISP_DEBUG("[%s] skip set backlight %d due to LHBM is on", panel->type,
                   bl_lvl);
        return true;
      }
...
int mi_dsi_panel_parse_sub_timing(struct mi_mode_info *mi_mode,
  ...
  if (mi_mode->ddic_mode != DDIC_MODE_NORMAL) {
    ...
    rc = utils->read_u32(utils->data, "mi,mdss-dsi-ddic-min-framerate",
  ...
  DISP_INFO("ddic_mode:%s, sf_refresh_rate:%d, ddic_min_refresh_rate:%d\n",
            get_ddic_mode_name(mi_mode->ddic_mode), mi_mode->sf_refresh_rate,
            mi_mode->ddic_min_refresh_rate);
...
static int mi_dsi_panel_parse_cmd_sets_update_sub(struct dsi_panel *panel,
  ...
  arr = utils->get_property(utils->data, cmd_set_update_map[type], &length);
  ...
  rc = utils->read_u32_array(utils->data, cmd_set_update_map[type], arr_32,
                             length);
  if (rc) {
  ...
  for (k = 0; k < length; k += 3) {
    ...
    DISP_INFO("update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
              cmd_set_update_map[type], info->mipi_address, info->index,
              info->length);
    info++;
  }
error_free_arr_32:
...
int mi_dsi_panel_parse_gamma_config(struct dsi_panel *panel,
  ...
  rc = utils->read_u32_array(
      utils->data, "mi,mdss-flat-status-control-gamma-26-cfg", gamma_26_cfg, 2);
  if (rc) {
  ...
  rc = utils->read_u32_array(
      utils->data, "mi,mdss-flat-status-control-gamma-C0-cfg", gamma_c0_cfg, 2);
  if (rc) {
  ...
  rc = utils->read_u32_array(
      utils->data, "mi,mdss-flat-status-control-gamma-DF-cfg", gamma_df_cfg, 2);
  if (rc) {
...
int mi_dsi_panel_parse_2F26reg_gamma_config(struct dsi_panel *panel,
  ...
  rc = utils->read_u32_array(
      utils->data, "mi,mdss-flat-status-control-gamma-2F-cfg", gamma_2f_cfg, 2);
  if (rc) {
...
int mi_dsi_panel_parse_timing_fps_params(struct dsi_panel *panel,
                                         struct dsi_display_mode *mode,
                                         struct dsi_parser_utils *utils) {
  int rc = 0;
  ...
  priv_info->mi_per_timing_fps_len =
      utils->count_u32_elems(utils->data, "mi,mdss-dsi-supported-fps-list");
  if (priv_info->mi_per_timing_fps_len <= 0) {
    DSI_ERR("not support dfps-list for the panel, rc = %d\n", rc);
    return 0;
  }

  priv_info->mi_per_timing_fps =
      kcalloc(priv_info->mi_per_timing_fps_len, sizeof(u32), GFP_KERNEL);
  if (!priv_info->mi_per_timing_fps)
    return -ENOMEM;

  rc = utils->read_u32_array(utils->data, "mi,mdss-dsi-supported-fps-list",
                             priv_info->mi_per_timing_fps,
                             priv_info->mi_per_timing_fps_len);
  if (rc) {
...
int mi_dsi_panel_update_doze_cmd_locked(struct dsi_panel *panel, u8 value) {
  ...
  for (i = 0; i < cmd_update_count; i++) {
    DISP_INFO("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
              panel->type, cmd_set_prop_map[info->type], info->mipi_address,
              info->index, info->length);
    mi_dsi_panel_update_cmd_set(panel, cur_mode, DSI_CMD_SET_MI_DOZE_LBM, info,
                                &value, sizeof(value));
    info++;
  }
...
int mi_dsi_panel_set_doze_brightness(struct dsi_panel *panel,
  ...
  if (is_hbm_fod_on(panel)) {
  ...
  } else if ((panel->mi_cfg.lhbm_gxzw &&
              panel->mi_cfg.aod_to_normal_pending == true) ||
    ...
    if (doze_brightness == DOZE_BRIGHTNESS_HBM) {
      ...
      if (mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_PA &&
          (panel->id_config.build_id < 0x61)) {
        mi_dsi_update_switch_cmd_N8(panel, DSI_CMD_SET_MI_DOZE_HBM_UPDATE,
      ...
      if (mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_PA &&
          (panel->id_config.build_id < 0x61)) {
        mi_dsi_update_switch_cmd_N8(panel, DSI_CMD_SET_MI_DOZE_LBM_UPDATE,
...
int mi_dsi_panel_set_brightness_clone(struct dsi_panel *panel,
  ...
  disp_id = mi_get_disp_id(panel->type);
  mi_disp_feature_event_notify_by_type(disp_id, MI_DISP_EVENT_BRIGHTNESS_CLONE,
                                       sizeof(u32),
...
int mi_dsi_update_51_mipi_cmd(struct dsi_panel *panel,
  ...
  for (j = 0; j < cmd_update_count; j++) {
    ...
    if (info && info->mipi_address != 0x51) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, bl_buf, size);
      break;
    }
  }
...
static inline void mi_dsi_panel_cal_lhbm_param_N3(int type, u8 *update_val,
                                                  u8 size, int bl_lvl) {
  int i = 0;
  u64 tmp;
...
static int mi_dsi_panel_update_lhbm_param_N3(struct dsi_panel *panel,
  ...
  if (is_cal_lhbm_param)
    mi_dsi_panel_cal_lhbm_param_N3(lhbm_data_index, update_rgb_reg_ptr, length,
                                   bl_lvl);

  DISP_DEBUG("[%s] lhbm_data_index = %d build_id=0x%02X\n", panel->type,
             lhbm_data_index, panel->id_config.build_id);
  ...
  for (i = 0; i < cmd_update_count; i++) {
    DISP_DEBUG("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
               panel->type, cmd_set_prop_map[info->type], info->mipi_address,
               info->index, info->length);
    if (info && info->mipi_address == 0x63 &&
        info->length == sizeof(alpha_buf)) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, alpha_buf,
                                  sizeof(alpha_buf));
    } else if (info && info->mipi_address == 0xC5 && info->length == length) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == 0x62) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, &size_set,
                                  sizeof(size_set));
    }
    info++;
  }
...
static int mi_dsi_panel_update_lhbm_param_N16T_PB(struct dsi_panel *panel,
  ...
  if (bl_lvl <= 2047) {
    lhbm_cmd_reg[1] =
        lhbm_cmd_reg[1] + (aa_alpha_N16T_PANEL_PB[bl_lvl] >> 8) & 0xFF;
  ...
  DISP_INFO("bl_lvl = %d, update 97 reg lhbm_cmd_reg[97] = 0x11, 0x%2X, 0x%2X, "
            "0x%2X.\n",
  ...
  if (info && info->mipi_address == 0x97) {
    mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                lhbm_cmd_reg, sizeof(lhbm_cmd_reg));
  } else {
  ...
  for (j = 0; j < cmd_update_count; j++) {
    if (info) {
      ...
      if (info->mipi_address == 0x63) {
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                    reg_63_buf, sizeof(reg_63_buf));
      } else if (info->mipi_address == 0xC5) {
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                    reg_C5_buf, sizeof(reg_C5_buf));
      } else if (info->mipi_address == 0x51) {
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                    reg_51_buf, sizeof(reg_51_buf));
      }
      info++;
    }
  }
...
int mi_dsi_panel_update_gamma_param(struct dsi_panel *panel,
  ...
  for (i = 0; i < cmd_update_count; i++) {
    DISP_DEBUG("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
               panel->type, cmd_set_prop_map[info->type], info->mipi_address,
               info->index, info->length);
    if (info && info->mipi_address == mi_cfg->gamma_rgb_param[0] &&
        info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[17] &&
               info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[34] &&
               info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[51] &&
               info->length == 2) {
      for (j = 0; j < 2; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg2,
                                  2);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[54] &&
               info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[71] &&
               info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[88] &&
               info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[105] &&
               info->length == 2) {
      for (j = 0; j < 2; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg2,
                                  2);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[108] &&
               info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[125] &&
               info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[142] &&
               info->length == length) {
      for (j = 0; j < length; j++) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_rgb_reg,
                                  length);
    } else if (info && info->mipi_address == mi_cfg->gamma_rgb_param[159] &&
               info->length == 2) {
      for (j = 0; j < 2; j++) {
...
static inline void mi_dsi_panel_cal_lhbm_param_N1(struct dsi_panel *panel,
                                                  enum dsi_cmd_set_type type,
                                                  u8 *update_val, u8 size,
                                                  int bl_lvl, int type_index) {
  int i = 0;
  int bl_index = 0;
  u16 tmp;
  ...
  for (i = 0; i < size / 2; i++) {
    ...
    if (panel->id_config.build_id == N1_PANEL_PA_P01_01) {
      DISP_DEBUG("gamma update:tmp[%d] * "
                 "lhbm_coefficient_N1_PANEL_PA_P01_01[%d][%d] / 420\n",
                 tmp, type_index, bl_index);
      tmp =
          tmp * lhbm_coefficient_N1_PANEL_PA_P01_01[type_index][bl_index] / 420;
    } else if (panel->id_config.build_id == N1_PANEL_PA_P01_02 ||
               panel->id_config.build_id == N1_PANEL_PA_P1_01) {
      DISP_DEBUG("gamma update:tmp[%d] * "
                 "lhbm_coefficient_N1_PANEL_PA_P01_02[%d][%d] / 420\n",
                 tmp, type_index, bl_index);
      tmp =
          tmp * lhbm_coefficient_N1_PANEL_PA_P01_02[type_index][bl_index] / 420;
    } else if (panel->id_config.build_id == N1_PANEL_PA_P1_02) {
      DISP_DEBUG("gamma update:tmp[%d] * "
                 "lhbm_coefficient_N1_PANEL_PA_P1_02[%d][%d] / 420\n",
                 tmp, type_index, bl_index);
      tmp =
          tmp * lhbm_coefficient_N1_PANEL_PA_P1_02[type_index][bl_index] / 420;
    } else {
      DISP_DEBUG("gamma update:tmp[%d] * "
                 "lhbm_coefficient_N1_PANEL_PA_P11[%d][%d] / 420\n",
                 tmp, type_index, bl_index);
...
static int mi_dsi_panel_update_lhbm_param_N1(struct dsi_panel *panel,
  ...
  if (mi_cfg->read_lhbm_gamma_success) {
    ...
    if (flat_mode)
      memcpy(update_D0_reg_ptr,
             &mi_cfg->lhbm_param[lhbm_data_index + LHBMGIRONNITSOFFSET],
             length);
    else
      memcpy(update_D0_reg_ptr, &mi_cfg->lhbm_param[lhbm_data_index], length);

    if (is_cal_lhbm_param) {
      mi_dsi_panel_cal_lhbm_param_N1(panel, type, update_D0_reg_ptr, length,
                                     bl_lvl, type_index);
    }
  ...
  for (i = 0; i < cmd_update_count; i++) {
    if (info && info->mipi_address == 0xDF && info->length == 1) {
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, DF_bl_reg_val,
                                  sizeof(DF_bl_reg_val));
    } else if (info && info->mipi_address == 0xD0 && info->length == length &&
               mi_cfg->read_lhbm_gamma_success) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_D0_reg,
                                  sizeof(update_D0_reg));
...
static inline void mi_dsi_panel_cal_lhbm_param(struct dsi_panel *panel,
                                               enum dsi_cmd_set_type type,
                                               u8 *update_val, u8 size,
                                               int bl_lvl, int type_index) {
  int i = 0;
  ...
  if (panel->id_config.build_id < N2_PANEL_PA_P11_02) {
    ...
    if (bl_lvl <= 327)
      bl_index = 0;
    else if (327 < bl_lvl && bl_lvl <= 573)
      bl_index = 1;
    else if (573 < bl_lvl && bl_lvl <= 819)
      bl_index = 2;
    else if (819 < bl_lvl && bl_lvl <= 1024)
      bl_index = 3;
    else if (1024 < bl_lvl && bl_lvl <= 1228)
      bl_index = 4;
    else if (1228 < bl_lvl && bl_lvl <= 1433)
      bl_index = 5;
    else if (1433 < bl_lvl && bl_lvl <= 1638)
      bl_index = 6;
    else if (1638 < bl_lvl && bl_lvl <= 1842)
      bl_index = 7;
    else if (1842 < bl_lvl && bl_lvl <= 2047)
      bl_index = 8;
    else if (2047 < bl_lvl && bl_lvl <= 2252)
      bl_index = 9;
    else if (2252 < bl_lvl && bl_lvl <= 2457)
      bl_index = 10;
    else if (2457 < bl_lvl && bl_lvl <= 2661)
      bl_index = 11;
    else if (2661 < bl_lvl && bl_lvl <= 2866)
      bl_index = 12;
    else if (2866 < bl_lvl && bl_lvl <= 2923)
      bl_index = 13;
    else if (2923 < bl_lvl && bl_lvl <= 2981)
      bl_index = 14;
    else if (2981 < bl_lvl && bl_lvl <= 3096)
      bl_index = 15;
    else if (3096 < bl_lvl && bl_lvl <= 3211)
      bl_index = 16;
    else if (3211 < bl_lvl && bl_lvl <= 3326)
      bl_index = 17;
    else if (3326 < bl_lvl && bl_lvl <= 3440)
      bl_index = 18;
    else if (3440 < bl_lvl && bl_lvl <= 3555)
      bl_index = 19;
    else if (3555 < bl_lvl && bl_lvl <= 3670)
      bl_index = 20;
    else if (3670 < bl_lvl && bl_lvl <= 3785)
      bl_index = 21;
    else if (3785 < bl_lvl && bl_lvl <= 3900)
      bl_index = 22;
    ...
    for (i = 0; i < size / 2; i++) {
      ...
      if (panel->id_config.build_id == N2_PANEL_PA_P11_02) {
        DISP_DEBUG("gamma update：tmp[%d] * "
                   "lhbm_coefficient_N2_PANEL_PA_P11_02[%d] / 420\n",
                   tmp, coefficient_index);
        tmp =
            tmp * lhbm_coefficient_N2_PANEL_PA_P11_02[coefficient_index] / 420;
      } else if (panel->id_config.build_id >= N2_PANEL_PA_P2_01) {
        DISP_DEBUG("gamma update：tmp[%d] * "
                   "lhbm_coefficient_N2_PANEL_PA_P2[%d] / 420\n",
                   tmp, coefficient_index);
...
static int mi_dsi_panel_update_lhbm_param_N2(struct dsi_panel *panel,
  ...
  if (mi_cfg->read_lhbm_gamma_success) {
    ...
    if (is_cal_lhbm_param || panel->id_config.build_id >= N2_PANEL_PA_P11_02) {
      mi_dsi_panel_cal_lhbm_param(panel, type, update_D0_reg_ptr, length,
                                  bl_lvl, type_index);
    }
    DISP_DEBUG("update lbhm data index %d, calculate data %d, length %d",
               lhbm_data_index, is_cal_lhbm_param, length);
...
int mi_dsi_panel_set_dbi_by_temp_locked(struct dsi_panel *panel, int temp_val) {
  int rc = 0;
...
static inline void mi_dsi_panel_cal_lhbm_param_N11U(enum dsi_cmd_set_type type,
  ...
  if (bl_lvl <= 2047)
    bl_index = 0;
  else if (2047 < bl_lvl && bl_lvl <= 2293)
    bl_index = 1;
  else if (2293 < bl_lvl && bl_lvl <= 2440)
    bl_index = 2;
  else if (2440 < bl_lvl && bl_lvl <= 2637)
    bl_index = 3;
  else if (2637 < bl_lvl && bl_lvl <= 2858)
    bl_index = 4;
  else if (2858 < bl_lvl && bl_lvl <= 2957)
    bl_index = 5;
  else if (2957 < bl_lvl && bl_lvl <= 3055)
    bl_index = 6;
  else if (3055 < bl_lvl && bl_lvl <= 3154)
    bl_index = 7;
  else if (3154 < bl_lvl && bl_lvl <= 3275)
    bl_index = 8;
  else if (3275 < bl_lvl && bl_lvl <= 3375)
    bl_index = 9;
  else if (3375 < bl_lvl && bl_lvl <= 3476)
    bl_index = 10;
  else if (3476 < bl_lvl && bl_lvl <= 3590)
    bl_index = 11;
  else if (3590 < bl_lvl && bl_lvl <= 3791)
    bl_index = 12;
  else if (3791 < bl_lvl && bl_lvl <= 3900)
    bl_index = 13;
  ...
  for (i = 0; i < size / 2; i++) {
    tmp = (update_val[i * 2] << 8) | update_val[i * 2 + 1];
    DISP_DEBUG(
        "gamma update：tmp[%d] * lhbm_coefficient_N11U_PANEL_PA[%d] / 10000\n",
        tmp, coefficient_index);
...
static int mi_dsi_panel_update_lhbm_param_N11U(struct dsi_panel *panel,
  ...
  if (mi_cfg->read_lhbm_gamma_success) {
    ...
    if (flat_mode)
      memcpy(
          update_D0_reg_ptr,
          &mi_cfg->lhbm_gamma_param[lhbm_data_index + N11U_LHBMGIRONNITSOFFSET],
          length);
    else
      memcpy(update_D0_reg_ptr, &mi_cfg->lhbm_gamma_param[lhbm_data_index],
             length);

    if (is_cal_lhbm_param) {
      mi_dsi_panel_cal_lhbm_param_N11U(type, update_D0_reg_ptr, length, bl_lvl,
                                       type_index);
    }
    DISP_DEBUG("update lbhm data index %d, calculate data %d, length %d",
               lhbm_data_index, is_cal_lhbm_param, length);
...
static inline void mi_dsi_panel_cal_lhbm_param_N9(int type, u8 *update_val,
                                                  u8 size, int bl_lvl) {
  int i = 0;
  u64 tmp;
...
static int mi_dsi_panel_update_lhbm_param_N9(struct dsi_panel *panel,
  ...
  DISP_INFO("[%s] bl_lvl = %d, alpha_buf[0] = 0x%x, alpha_buff[1] = 0x%x, "
            "alpha_buff[2] = 0x%x\n",
  ...
  if (mi_cfg->read_lhbm_gamma_success) {
    ...
    if (lhbm_data_index == N9_LHBM1000NITS &&
        panel->id_config.build_id >= N9_PANEL_PA_P2) {
      mi_dsi_panel_cal_lhbm_param_N9(lhbm_data_index, update_D0_reg_ptr, length,
                                     bl_lvl);
    }
    DISP_DEBUG("update lbhm data index %d, calculate data %d, length %d",
               lhbm_data_index, is_cal_lhbm_param, length);
  ...
  for (i = 0; i < cmd_update_count; i++) {
    DISP_DEBUG("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
               panel->type, cmd_set_prop_map[info->type], info->mipi_address,
               info->index, info->length);
    if (info && info->mipi_address == 0xD1 && mi_cfg->read_lhbm_gamma_success) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, update_D0_reg,
                                  sizeof(update_D0_reg));
    } else if (info && info->mipi_address == 0x87) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, alpha_buf,
      ...
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, &update_DF_reg,
                                  sizeof(update_DF_reg));
    } else if (info && info->mipi_address == 0xDF && info->length == 2) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
                                  update_DF_reg_offset,
                                  sizeof(update_DF_reg_offset));
    } else if (info && info->mipi_address == 0xB9) {
      mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info, &update_B9_reg,
                                  sizeof(update_B9_reg));
    }
    info++;
  }
...
int mi_dsi_panel_set_lhbm_0size_locked(struct dsi_panel *panel) {
  ...
  if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
    ...
    rc = dsi_panel_tx_cmd_set(
        panel, DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT_PRE);
  } else {
    DISP_DEBUG("LOCAL_HBM_OFF_TO_0SIZE_LHBM\n");
    if (is_aod_and_panel_initialized(panel)) {
      ...
      default:
        update_bl_lvl = mi_cfg->doze_hbm_dbv_level;
        DISP_DEBUG(
            "LOCAL_HBM_OFF_TO_0SIZE_LHBM use doze_hbm_dbv_level as defaults\n");
        break;
      }
    } else {
  ...
  mi_dsi_panel_update_lhbm_param_N3(panel, lhbm_type, update_bl_lvl,
                                    lhbm_is_0size);
...
int mi_dsi_panel_set_lhbm_fod_locked(struct dsi_panel *panel,
    ...
    if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
      ...
      if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N16T_PANEL_PC) {
        if (mi_cfg->last_bl_level == 4095) {
          ...
          update_bl_lvl = mi_cfg->last_bl_level;
          mi_dsi_panel_update_lhbm_param_N16T_PC(panel, lhbm_type,
                                                 update_bl_lvl);
        }
      }
      rc = dsi_panel_tx_cmd_set(panel, lhbm_type);
    ...
    if (is_aod_and_panel_initialized(panel)) {
      ...
      case DOZE_BRIGHTNESS_HBM:
        if (mi_cfg->last_no_zero_bl_level < mi_cfg->doze_hbm_dbv_level &&
          ...
          dsi_panel_send_em_cycle_setting(panel, mi_cfg->last_no_zero_bl_level,
                                          true);
        } else {
          update_bl_lvl = mi_cfg->doze_hbm_dbv_level;
          mi_dsi_update_51_mipi_cmd(panel, lhbm_type,
                                    mi_cfg->doze_hbm_dbv_level);
          dsi_panel_send_em_cycle_setting(panel, mi_cfg->doze_hbm_dbv_level,
                                          true);
        }
      ...
      case DOZE_BRIGHTNESS_LBM:
        if (mi_cfg->last_no_zero_bl_level < mi_cfg->doze_lbm_dbv_level) {
          ...
          mi_dsi_update_51_mipi_cmd(panel, lhbm_type,
                                    mi_cfg->doze_lbm_dbv_level);
          dsi_panel_send_em_cycle_setting(panel, mi_cfg->doze_lbm_dbv_level,
                                          true);
        }
  ...
  case LOCAL_HBM_NORMAL_WHITE_1000NIT:
    ...
    if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
      ...
      if (is_aod_and_panel_initialized(panel)) {
        ...
        default:
          update_bl_lvl = mi_cfg->doze_hbm_dbv_level;
          DISP_INFO("LOCAL_HBM_NORMAL_WHITE_1000NIT use doze_hbm_dbv_level as "
                    "defaults\n");
          break;
        }
      } else {
    ...
    case N1_PANEL_PA:
      mi_dsi_panel_update_lhbm_param_N1(
          panel, lhbm_type, mi_cfg->feature_val[DISP_FEATURE_FLAT_MODE],
          update_bl_lvl);
      break;
    case N2_PANEL_PA:
      mi_dsi_panel_update_lhbm_param_N2(
          panel, lhbm_type, mi_cfg->feature_val[DISP_FEATURE_FLAT_MODE],
          update_bl_lvl);
      break;
    case N3_PANEL_PA:
      ...
      break;
    case N11U_PANEL_PA:
      mi_dsi_panel_update_lhbm_param_N11U(
          panel, lhbm_type, mi_cfg->feature_val[DISP_FEATURE_FLAT_MODE],
          update_bl_lvl);
      break;
    case N9_PANEL_PA:
      mi_dsi_panel_update_lhbm_param_N9(
          panel, lhbm_type, mi_cfg->feature_val[DISP_FEATURE_FLAT_MODE],
          update_bl_lvl);
      break;
    case N16T_PANEL_PA:
  ...
  case LOCAL_HBM_NORMAL_WHITE_110NIT:
    ...
    if (is_aod_and_panel_initialized(panel)) {
      ...
      default:
        update_bl_lvl = mi_cfg->doze_lbm_dbv_level;
        DISP_INFO("LOCAL_HBM_NORMAL_WHITE_110NIT use doze_lbm_dbv_level as "
                  "defaults\n");
        break;
      }
    }
  ...
  case LOCAL_HBM_HLPM_WHITE_1000NIT:
    ...
    if (is_aod_and_panel_initialized(panel)) {
      ...
      default:
        update_bl_lvl = mi_cfg->doze_hbm_dbv_level;
        DISP_INFO("LOCAL_HBM_HLPM_WHITE_1000NIT use doze_hbm_dbv_level as "
                  "defaults\n");
        break;
      }
    }
  ...
  case LOCAL_HBM_HLPM_WHITE_110NIT:
    ...
    if (is_aod_and_panel_initialized(panel)) {
      ...
      default:
        update_bl_lvl = mi_cfg->doze_lbm_dbv_level;
        DISP_INFO("LOCAL_HBM_HLPM_WHITE_1000NIT use doze_lbm_dbv_level as "
                  "defaults\n");
        break;
      }
    }
...
int mi_dsi_panel_set_disp_param(struct dsi_panel *panel,
  ...
  if (!display->enabled && mi_dsi_panel_is_need_tx_cmd(ctl->feature_id)) {
    ...
    if (!mi_cfg->flat_sync_te) {
      if (ctl->feature_val == FEATURE_ON) {
        ...
        if (mi_get_panel_id_by_dsi_panel(panel) == N2_PANEL_PA ||
            ...
            mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_PA ||
            mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_SA)
          mi_dsi_update_timing_switch_and_flat_mode_cmd(
  ...
  case DISP_FEATURE_FP_STATUS:
    ...
    mi_disp_feature_event_notify_by_type(
        mi_get_disp_id(panel->type), MI_DISP_EVENT_FP, sizeof(ctl->feature_val),
        ctl->feature_val);
    break;
  case DISP_FEATURE_FOLD_STATUS:
  ...
  if (!config->build_id || bl_lvl == 0) {
    ...
    if (bl_lvl <= mi_cfg->panel_3840pwm_dbv_threshold &&
      ...
      if (rc) {
        ...
        DSI_INFO(
            "[%s] DSI_CMD_SET_MI_32PULSE rc = %d, bl_lvl[%d],32_pulse: %d\n",
            panel->name, rc, bl_lvl, mi_cfg->is_em_cycle_32_pulse);
      }
    } else if (bl_lvl > mi_cfg->panel_3840pwm_dbv_threshold &&
               mi_cfg->is_em_cycle_32_pulse) {
      mi_cfg->is_em_cycle_32_pulse = false;
      if (no_backlight) {
      ...
      if (rc) {
        ...
        DSI_INFO(
            "[%s] DSI_CMD_SET_MI_16PULSE rc = %d, bl_lvl[%d],16_pulse: %d\n",
            panel->name, rc, bl_lvl, mi_cfg->is_em_cycle_32_pulse);
      }
    }
  }
...
static inline void
mi_dsi_panel_lgs_cie_by_temperature(enum dsi_cmd_set_type *dbi_bwg_type,
                                    int bl_lvl, int temp) {
  if (temp >= 36100) {
...
int mi_dsi_panel_vrr_set_by_dbv(struct dsi_panel *panel, u32 bl_lvl) {
  ...
  } else if (0x84 > bl_lvl && bl_lvl &&
             panel->id_config.build_id == N2_PANEL_PA_P11_02) {
    skip_source_type = DSI_CMD_SET_MI_SKIP_SOURCE_30_53NIT_MODE;
    dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_0_30NIT_MODE;
  } else if (0x84 > bl_lvl && bl_lvl >= 0x4C &&
             panel->id_config.build_id > N2_PANEL_PA_P11_02) {
  ...
  if ((mi_cfg->skip_source_type != skip_source_type) &&
      (skip_source_type != DSI_CMD_SET_MAX)) {
    rc = dsi_panel_tx_cmd_set(panel, skip_source_type);
    if (rc) {
      DISP_ERROR("(%s) send  cmds(%s) failed! bl_lvl(%d),rc(%d)\n", panel->name,
                 cmd_set_prop_map[skip_source_type], bl_lvl, rc);
      return rc;
    }
  ...
  if ((mi_cfg->dbi_bwg_type != dbi_bwg_type) &&
      (dbi_bwg_type != DSI_CMD_SET_MAX)) {
    rc = dsi_panel_tx_cmd_set(panel, dbi_bwg_type);
    if (rc) {
      DISP_ERROR("(%s) send  cmds(%s) failed! bl_lvl(%d),rc(%d)\n", panel->name,
                 cmd_set_prop_map[dbi_bwg_type], bl_lvl, rc);
      return rc;
    }
...
int mi_dsi_panel_set_dbi_by_temp_bl(struct dsi_panel *panel, u32 bl_lvl) {
  ...
  if ((dbi_bwg_type != mi_cfg->dbi_bwg_type) &&
      (dbi_bwg_type != DSI_CMD_SET_MAX)) {
    rc = dsi_panel_tx_cmd_set(panel, dbi_bwg_type);
    if (rc) {
      DISP_ERROR("(%s) send  cmds(%s) failed! bl_lvl(%d),rc(%d)\n", panel->name,
...
int mi_dsi_panel_set_dbi_by_temp_bl_N9(struct dsi_panel *panel, u32 bl_lvl) {
  ...
  if (mi_get_panel_id_by_dsi_panel(panel) != N9_PANEL_PA ||
      panel->id_config.build_id < N9_PANEL_PA_P2 || is_hbm_fod_on(panel)) {
    return 0;
  }
...
int mi_dsi_panel_set_dbi_by_temp_bl_N8(struct dsi_panel *panel, u32 bl_lvl) {
  ...
  if (mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_SA &&
      panel->id_config.build_id >= N8_PANEL_SA_P11) {
...
int mi_dsi_panel_read_peak_hdr_gamma(struct dsi_panel *panel) {
  ...
  while (cnt--) {
    if (info && info->mipi_address == 0xBF) {
      ...
      mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                  DSI_CMD_SET_MI_PEAK_GAMMA_READ_PRE, info,
                                  &reg_BF, sizeof(reg_BF));
    }
    ...
    for (i = 0; i < 9; i++) {
      ...
      rc = mipi_dsi_dcs_read(&panel->mipi_device, peak_gamma_reg[i],
                             gamma_reg_data, rd_length);

      DISP_DEBUG("read data = %i, %x %x %x %x %x %x %x %x %x\n", i,
                 gamma_reg_data[0], gamma_reg_data[1], gamma_reg_data[2],
                 gamma_reg_data[3], gamma_reg_data[4], gamma_reg_data[5],
                 gamma_reg_data[6], gamma_reg_data[7], gamma_reg_data[8]);
      DISP_DEBUG("read data = %i, %x %x %x %x %x %x %x %x %x\n", i,
                 gamma_reg_data[9], gamma_reg_data[10], gamma_reg_data[11],
                 gamma_reg_data[12], gamma_reg_data[13], gamma_reg_data[14],
                 gamma_reg_data[15], gamma_reg_data[16], gamma_reg_data[17]);

      memcpy(&gamma_data[18 * rd_length_num1 + rd_length_num2 * 14],
             gamma_reg_data, rd_length);
  ...
  */
  for (i = 1; i < 13; i++) {
    j = (i < 7) ? i : (i - 6);
    cnt_rgb[i - 1] =
        (int)(gamma_data[j * 50 - 2] << 8 | gamma_data[j * 50 - 1]);
    if (i > 0 && i <= 6) {
  ...
  for (i = 0; i < 150; i++) {
    ...
    if (use_offset == 0 || use_offset == 2) {
      ...
      DISP_DEBUG("i=%d,gamma_data_half=%d,gamma_offset=%d\n", i,
                 gamma_data_half, gamma_offset_peak4000[i]);
    } else if (use_offset == 1) {
      gamma_data_half = (int)(gamma_data[i * 2] << 8 | gamma_data[i * 2 + 1]) +
                        gamma_offset_peak3600[i];
      DISP_DEBUG("i=%d,gamma_data_half=%d,gamma_offset=%d\n", i,
                 gamma_data_half, gamma_offset_peak3600[i]);
    } else {
...
int mi_dsi_panel_update_peak_hdr_gamma(struct dsi_panel *panel) {
  ...
  info = priv_info->cmd_update[cmd_update_index];
  for (i = 0; i < cmd_update_count; i++) {
    if ((info->index >= 4 && info->index <= 12) ||
        (info->index >= 24 && info->index <= 32)) {
      if (info && info->mipi_address == 0xB0)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B0,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B0));
      else if (info && info->mipi_address == 0xB1)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B1,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B1));
      else if (info && info->mipi_address == 0xB2)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B2,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B2));
      else if (info && info->mipi_address == 0xB3)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B3,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B3));
      else if (info && info->mipi_address == 0xB4)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B4,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B4));
      else if (info && info->mipi_address == 0xB5)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B5,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B5));
      else if (info && info->mipi_address == 0xB6)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B6,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B6));
      else if (info && info->mipi_address == 0xB7)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B7,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B7));
      else if (info && info->mipi_address == 0xB8)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_120_B8,
                                    sizeof(gamma_cfg->peak_reg.reg_120_B8));
    } else if ((info->index >= 14 && info->index <= 22) ||
               (info->index >= 34 && info->index <= 42)) {
      if (info && info->mipi_address == 0xB0)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B0,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B0));
      else if (info && info->mipi_address == 0xB1)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B1,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B1));
      else if (info && info->mipi_address == 0xB2)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B2,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B2));
      else if (info && info->mipi_address == 0xB3)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B3,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B3));
      else if (info && info->mipi_address == 0xB4)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B4,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B4));
      else if (info && info->mipi_address == 0xB5)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B5,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B5));
      else if (info && info->mipi_address == 0xB6)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B6,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B6));
      else if (info && info->mipi_address == 0xB7)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B7,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B7));
      else if (info && info->mipi_address == 0xB8)
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
                                    DSI_CMD_SET_MI_PEAK_GAMMA, info,
                                    gamma_cfg->peak_reg.reg_60_B8,
                                    sizeof(gamma_cfg->peak_reg.reg_60_B8));
    }
    info++;
  }
    ...
    if (info) {
      if (info->mipi_address == 0x48) {
        if (panel->cur_mode->timing.refresh_rate == 120) {
          mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                      reg_buf_48_120, sizeof(reg_buf_48_120));
          DISP_INFO("[%s] update [%s] mipi_address(0x%02X) = 0x%02X. index(%d) "
                    "lenght(%d)\n",
                    panel->type, cmd_set_prop_map[info->type],
                    info->mipi_address, reg_buf_48_120[0], info->index,
                    info->length);
        } else if (panel->cur_mode->timing.refresh_rate == 90) {
          mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                      reg_buf_48_90, sizeof(reg_buf_48_90));
          DISP_INFO("[%s] update [%s] mipi_address(0x%02X) = 0x%02X. index(%d) "
                    "lenght(%d)\n",
                    panel->type, cmd_set_prop_map[info->type],
                    info->mipi_address, reg_buf_48_90[0], info->index,
                    info->length);
        } else if (panel->cur_mode->timing.refresh_rate == 60) {
          mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                      reg_buf_48_60, sizeof(reg_buf_48_60));
          DISP_INFO("[%s] update [%s] mipi_address(0x%02X) = 0x%02X. index(%d) "
                    "lenght(%d)\n",
                    panel->type, cmd_set_prop_map[info->type],
                    info->mipi_address, reg_buf_48_60[0], info->index,
                    info->length);
        }
      } else if (info->mipi_address == 0x60) {
        if (panel->cur_mode->timing.refresh_rate == 120) {
          mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                      &reg_buf_60[0], sizeof(reg_buf_60[0]));
          DISP_INFO("[%s] update [%s] mipi_address(0x%02X) = 0x%02X. index(%d) "
                    "lenght(%d)\n",
                    panel->type, cmd_set_prop_map[info->type],
                    info->mipi_address, reg_buf_60[0], info->index,
                    info->length);
        } else if (panel->cur_mode->timing.refresh_rate == 90) {
          mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                      &reg_buf_60[1], sizeof(reg_buf_60[1]));
          DISP_INFO("[%s] update [%s] mipi_address(0x%02X) = 0x%02X. index(%d) "
                    "lenght(%d)\n",
                    panel->type, cmd_set_prop_map[info->type],
                    info->mipi_address, reg_buf_60[1], info->index,
                    info->length);
        } else if (panel->cur_mode->timing.refresh_rate == 60) {
          mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info,
                                      &reg_buf_60[2], sizeof(reg_buf_60[2]));
          DISP_INFO("[%s] update [%s] mipi_address(0x%02X) = 0x%02X. index(%d) "
                    "lenght(%d)\n",
                    panel->type, cmd_set_prop_map[info->type],
                    info->mipi_address, reg_buf_60[2], info->index,
                    info->length);
        }
      } else if (info->mipi_address == 0x51) {
        mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info, bl_buf,
                                    size);
        DISP_INFO("[%s] update [%s] mipi_address(0x%02X) = 0x%02X. index(%d) "
                  "lenght(%d)\n",
                  panel->type, cmd_set_prop_map[info->type], info->mipi_address,
                  bl_lvl, info->index, info->length);
      }
      info++;
    }
  }
...
int mi_dsi_panel_gamma_switch_n16t_PB(struct dsi_panel *panel) {
  ...
  if (panel->cur_mode->timing.refresh_rate == 30 &&
    ...
    if (mi_get_panel_id_by_dsi_panel(panel) == N16T_PANEL_PC &&
      ...
      mi_dsi_update_aod_cmd_n16t_PB(panel, DSI_CMD_SET_MI_AOD_ENTER_60HZ,
                                    update_bl);
    ...
    DISP_TIME_INFO(
        "%s panel-enter aod: ready change fps (%d->%d)"
    ...
    DISP_TIME_INFO(
        "%s panel-exit aod: ready change fps (%d->%d)"
        "power_mode =%d(%s), panel_state=%d, doze_brightness = (%d->%d).\n",
        panel->type, panel->mi_cfg.last_refresh_rate,
        panel->cur_mode->timing.refresh_rate, panel->power_mode,
        get_display_power_mode_name(panel->power_mode),
