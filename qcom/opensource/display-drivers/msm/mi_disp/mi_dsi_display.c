  ...
  if (cur_mode) {
    ...
    if (priv_info->phy_timing_len && priv_info->phy_timing_len == buf_size) {
      ...
      display_for_each_ctrl(i, dsi_display) {
        ...
        ret = dsi_phy_set_timing_params(ctrl->phy, priv_info->phy_timing_val,
...
static ssize_t mi_dsi_display_set_dsi_porch_rw_real(void *display,
  ...
  if (dsi_ctrl->host_config.panel_mode == DSI_OP_CMD_MODE) {
    dsi_ctrl->hw.ops.setup_cmd_stream(
        ...
        &dsi_ctrl->host_config.common_config, 0x0, &dsi_ctrl->roi);
  } else {
...
void mi_dsi_display_set_dsi_porch_rw(struct disp_display *dd_ptr,
  ...
  if (strncmp(opt, "set", 3) == 0) {
    ...
    ret = mi_dsi_display_set_dsi_porch_rw_real(dd_ptr->display, option,
                                               phy_timming_value);
  }
  return;
}
...
static ssize_t mi_dsi_display_get_dsi_phy_rw(struct dsi_display *dsi_display,
  ...
  for (i = 0; i < phy_len; i++) {
    if (i == phy_len - 1) {
      ...
      count += snprintf(buf + count, size - count, "0x%02X,", phy_timming[i]);
    }
  }
...
ssize_t mi_dsi_display_show_dsi_phy_rw(void *display, char *buf, size_t size) {
  ...
  if (cur_mode) {
    ...
    mi_dsi_display_get_dsi_phy_rw(dsi_display, phy_timing, buf, size,
                                  phy_timing_len);
    ret = 1;
  } else {
    ret = -EINVAL;
...
ssize_t mi_dsi_display_show_dsi_porch_rw(void *display, char *buf,
                                         size_t size) {
  ...
  while (token) {
    ...
    count +=
        snprintf(buf + count, size - count, "%s:%u\n", token, dsi_porch_ret);
    /* Removes leading whitespace from input_copy */
    if (p) {
      p = skip_spaces(p);
      token = strsep(&p, delim);
...
ssize_t mi_dsi_display_read_wp_info(void *display, char *buf, size_t size) {
  ...
  if (!strlen(wp_info_str)) {
    ...
    for (i = 0; i < panel->wp_config.wp_cmds_rlen -
         ...
         i++) {
      snprintf(
          wp_info_str + i * 2, size - (i * 2), "%02x",
          panel->wp_config.return_buf[i + panel->wp_config.wp_read_info_index]);
    }
  }
...
ssize_t mi_dsi_display_read_cell_id(void *display, char *buf, size_t size) {
  ...
  if (!strcmp(dsi_display->display_type, "primary")) {
    if (!strlen(cell_id_info_str)) {
      mi_dsi_acquire_wakelock(dsi_display->panel);
      ret = mi_dsi_display_read_ddic_cell_id(dsi_display, cell_id_read_buffer,
                                             size);
      mi_dsi_release_wakelock(dsi_display->panel);
...
MODULE_PARM_DESC(build_id, "msm_drm.build_id=<build id info> while <build id "
                           "info> is 'panel build id info' ");
